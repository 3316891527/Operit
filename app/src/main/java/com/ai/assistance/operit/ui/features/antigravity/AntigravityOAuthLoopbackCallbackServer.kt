...
internal class AntigravityOAuthLoopbackCallbackServer private constructor(
    ...
    suspend fun awaitCallback(): Uri = withContext(Dispatchers.IO) {
        ...
        throw IllegalStateException("Antigravity OAuth callback server stopped before receiving a callback")
    }
    ...
    private fun readCallbackUri(socket: Socket): Uri? {
        ...
        if (requestUri.path != AntigravityOAuthProtocol.CALLBACK_PATH) {
            return null
        }
    ...
    companion object {
        ...
        private val SUCCESS_RESPONSE = HttpResponse(
            status = "200 OK",
            body = "<html><body>Antigravity login complete. Return to Operit.</body></html>",
        )
        ...
        fun open(): AntigravityOAuthLoopbackCallbackServer {
            ...
            return AntigravityOAuthLoopbackCallbackServer(socket)
        }
    }
}