...
const qwenDraw = (function () {
    ...
    function getImageEditEndpoint(baseUrl: string): string {
        const trimmed = baseUrl.trim();
        if (!trimmed) return joinUrl(DEFAULT_API_BASE_URL, "api/v1/services/aigc/multimodal-generation/generation");

        if (trimmed.includes("/api/v1/services/aigc/multimodal-generation/generation")) return trimmed;
        ...
        if (trimmed.endsWith("/api/v1")) return joinUrl(trimmed, "services/aigc/multimodal-generation/generation");
        if (trimmed.endsWith("/api/v1/")) return joinUrl(trimmed, "services/aigc/multimodal-generation/generation");
    ...
    async function readLocalImageAsDataUrl(filePath: string): Promise<string> {
        ...
        const base64Content = binaryResult && (binaryResult as any).contentBase64
            ? String((binaryResult as any).contentBase64).trim()
            : "";
        if (!base64Content) {
    ...
    async function resolveImageReferences(imageUrls: unknown, imagePaths: unknown): Promise<string[]> {
    ...
    function extractImmediateImageUrl(parsed: unknown): string {
        ...
        const message = isRecord(firstChoice) && isRecord(firstChoice["message"]) ? firstChoice["message"] as Record<string, unknown> : null;
    ...
    }): Promise<{ task_id: string; image_url: string; effective_model: string; reference_count: number }> {
