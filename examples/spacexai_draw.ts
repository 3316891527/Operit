...
const spacexaiDraw = (function () {
    ...
    function getImageEditApiEndpoint(): string {
    ...
    function parseImageDataUrl(value: string): { base64: string; mime_type: string } | null {
        const raw = String(value || "").trim();
    ...
    function extractImageFromRecord(record: JsonMap, sourcePrefix: string): ImagePayloadResult | null {
    ...
    function extractImagePayload(payload: JsonMap): ImagePayloadResult | null {
        for (const arrayField of ["data", "images"]) {
            ...
            const result = isRecord(first)
                ? extractImageFromRecord(first as JsonMap, `${arrayField}[0]`)
    ...
    function summarizeFirstArrayItem(payload: JsonMap, field: string): string {
        const values = Array.isArray(payload[field]) ? payload[field] : [];
        if (values.length === 0) return "none";
        const first = values[0];
        if (isRecord(first)) return `keys=[${summarizeObjectKeys(first)}]`;
        return typeof first;
    }
    ...
    function buildImageResponseDiagnostic(payload: JsonMap, statusCode: number, endpoint: string): string {
        ...
        if (apiError) parts.push(`api_error=${apiError}`);
        return parts.join("; ");
    }
    function guessImageExtension(mimeType: string, base64: string): string {
    ...
    function buildImageReference(url: string): JsonMap {
        return {
            url,
            type: "image_url"
        };
    ...
    async function callXaiImageApi(params: ImageParams): Promise<ImagePayloadResult & { effective_model: string; reference_count: number }> {
        ...
        for (const field of ["aspect_ratio", "quality"] as const) {
        ...
        if (!response.isSuccessful()) {
            let failurePayload: JsonMap = {};
            try {
                const parsedFailure = JSON.parse(response.content) as unknown;
                if (isRecord(parsedFailure)) failurePayload = parsedFailure as JsonMap;
            } catch (_) {
