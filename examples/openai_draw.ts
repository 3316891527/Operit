...
const openaiDraw = (function () {
    ...
    function getImageEndpoint(baseUrl: string, kind: "generations" | "edits" = "generations"): string {
        const trimmed = baseUrl.trim();
        const path = kind === "edits" ? "images/edits" : "images/generations";
        if (!trimmed) return joinUrl(DEFAULT_API_BASE_URL, `v1/${path}`);

        if (trimmed.includes(`/v1/${path}`)) return trimmed;
        ...
        if (trimmed.endsWith("/v1")) return joinUrl(trimmed, path);
        if (trimmed.endsWith("/v1/")) return joinUrl(trimmed, path);
    ...
    function fileNameFromPath(filePath: string, fallbackIndex: number): string {
    ...
    async function parseOpenAIImageResponse(content: string, effectiveModel: string, referenceCount: number): Promise<OpenAIImageApiResult> {
        let parsed: any;
        try {
